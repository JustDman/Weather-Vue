<template>
  <div class="hourly">
    ...
    <div class="row">
      <div class="col-md-6 col-xs-6 text-center" v-for="(hourly, count) in forecast.hourly"
      v-bind:key="hourly.weather.icon">
          <div class="card text-white bg-secondary mb-3">
            <div class="card-header">Weather in {{address.name}} in {{count}} hours.</div>
            <div class="card-body">
              <h4 class="card-title" v-for="weather in hourly.weather" v-bind:key="weather.main">
                {{weather.main}}
              </h4>
              <p class="card-text icon-temp" v-for="weather in hourly.weather"
                v-bind:key="weather.description">
                {{weather.description}}<br />
                {{hourly.temp}} ℃
              </p>
              <p class="card-text" v-for="weather in hourly.weather" v-bind:key="weather.icon">
